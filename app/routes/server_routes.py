...
def start_server(server_id):
    ...
    if request.method == "GET":
        flash(
            "Please use the Start button on the server list to start the server.",
            "info",
        )
        return redirect(url_for("server.home"))
        ...
        if os.path.exists(eula_file_path):
            with SafeFileOperation(eula_file_path, "r") as eula_file:
                ...
                if "eula=true" not in eula_content:
                    ...
                    flash(
                        "You must accept the EULA before starting the server.",
                        "warning",
                    )
            ...
            raise FileOperationError(
                "EULA file not found. Please ensure the server is set up correctly."
            )
...
def backup_server(server_id):
    ...
    if request.method == "GET":
        flash(
            "Please use the Backup button on the server list to backup the server.",
            "info",
        )
        return redirect(url_for("server.home"))
