...
def validate_memory_allocation(requested_memory_mb, exclude_server_id=None):
    ...
    try:
        ...
        if new_total > config["max_total_mb"]:
            ...
            return (
                False,
                f"Total memory allocation would exceed limit. Available: {available}MB",
                available,
            )
...
def update_app_config(
    app_title=None, server_hostname=None, max_total_mb=None, max_per_server_mb=None
):
    ...
    try:
        ...
        if app_title is not None:
            ...
            if config_entry:
                ...
                config_entry = Configuration(
                    key="app_title", value=str(app_title), updated_by=user_id
                )
        ...
        if max_total_mb is not None:
            ...
            if config_entry:
                ...
                config_entry = Configuration(
                    key="max_total_mb", value=str(max_total_mb), updated_by=user_id
                )
                db.session.add(config_entry)
...
def get_memory_usage_summary(user_id=None):
    ...
    try:
        ...
        return {
            ...
            "usage_percentage": round((allocated / config["max_total_mb"]) * 100, 1)
            if config["max_total_mb"] > 0
            else 0,
        }
    except Exception as e:
...
def initialize_default_config():
    ...
    try:
        ...
        for key, value in default_config.items():
            ...
            if not existing:
                config_entry = Configuration(key=key, value=value, updated_by=None)
                db.session.add(config_entry)
    ...
    try:
        ...
        if process.is_running():
            try:
                ...
                if "java" in process_info["name"].lower() or any(
                    "java" in cmd.lower() for cmd in process_info["cmdline"]
                ):
                    return {
                        "is_running": True,
            ...
            try:
                ...
                if "java" in process.info["name"].lower():
                    ...
                    if (
                        cmdline
                        and len(cmdline) > 2
                        and "server.jar" in " ".join(cmdline)
                        and "nogui" in " ".join(cmdline)
                    ):
...
def periodic_status_check():
    ...
    summary = {"servers_checked": 0, "statuses_updated": 0, "errors": []}
