...
def handle_network_error(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            log_error(NetworkError(error_msg), {'function': func.__name__, 'status_code': e.response.status_code})
            raise NetworkError(error_msg)
...
def handle_database_operations(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            log_error(DatabaseError(error_msg), {'function': func.__name__, 'original_error': str(e)})
...
class SafeFileOperation:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type and issubclass(exc_type, Exception):
            raise FileOperationError(f"Error during file operation on {self.filepath}: {str(exc_val)}")
