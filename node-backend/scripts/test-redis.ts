...
class RedisTester {
  ...
  private async testPerformance(): Promise<void> {
    ...
    try {
      ...
      logger.info(
        `  Set operations: ${iterations} ops in ${setTime}ms (${((iterations / setTime) * 1000).toFixed(0)} ops/sec)`,
      );
      logger.info(
        `  Get operations: ${iterations} ops in ${getTime}ms (${((iterations / getTime) * 1000).toFixed(0)} ops/sec)`,
      );
